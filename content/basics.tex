...
\section{OpenCL}
\label{opencl}
OpenCL is a framework that represents a direct competitor to CUDA. It was initially proposed by Apple but is currently maintained by the Khronos Group, which consists of a multitude of companies like AMD, Intel, NVIDIA and others \cite{opencl_directors}. Unlike CUDA, OpenCL is not limited to NVIDIA GPUs but supports various device types and vendors. In fact OpenCL programs may be executed on CPUs, GPUs, FPGAs and digital signal processor (abbr. DSP). Prerequisite for the execution of OpenCL code on a targeted device is the presence of an OpenCL Installable Client Driver (abbr. ICD) on its enclosing host machine. Such ICDs must conform with the specifications released by the Khronos Group. As of today, various implementations of the specifications exist by vendors like AMD, NVIDIA, Intel, Apple, ARM, Xilinx and others \cite{opencl_conform}.

The most important definitions within the OpenCL standard are the introduction of its execution model as well as the underlying memory model, which are explained based on the OpenCL Specification Version 2.2 document \cite{opencl_spec}.

\subsection*{Execution Model}
OpenCL programs, called Kernels are executed on selected OpenCL devices. They are started from within a host program, which runs on the host that has the devices installed. A Kernel is run for each work-item, which is the smallest partial unit of a submitted task. For example a work-item could be an element of an array that is being processed. Work-items are bundled in work-groups that represent a subdivision of the overall processable data. in the OpenCL model devices consist of compute units (abbr. CU), which for instance for a CPU could be a CPU core. Each work-group is assigned to a CU that processes it. 

Through the execution model there exist two layers of parallelization. Firstly work-groups are independent of each other they can be executed in parallel when a device has more than one CU installed. Secondly the underlying CUs may be able to parallelize the execution for many work-items as well. This is entirely depending on the respective hardware capabilities and vendor implementation. For example modern AMD GPUs execute work-groups in so called wavefronts. A wavefront can contain up to 64 work-items, which are executed in batches of 16 items per clock cycle and thus take 4 clock cycles to cover all enclosed items\cite{amd_opencl_spec}. This means that an optimal work-group size for these devices is a multiple of 64 to ensure full utilization of a wavefront.
...
Having many work-groups assigned per CU also enables OpenCL to hide latencies for required memory accesses. Whenever a CU needs to access memory, which would stall its threads, the CU may switch to another processable work-group and continue execution until this work-group also stalls. Thus the time for memory accesses is spent computing instead of idling the CU.
...
\begin{description}[align=left,leftmargin=0cm]
  \item [Private] Only accessible by a work-item itself. Mainly used for temporary variables.
  \item [Local] Accessible by work-items within a work-group. Can be used to share and synchronize data within a group.
  \item [Global] Can be read by and written to by the host as well as all work-items.
  \item [Constant] Memory that is allocated and initialized by the host is read-only by work-items.
\end{description}
...
The following kernel code describes the summation of elements at identical indexes of two arrays:
...
For this example to run the host code has to choose an appropriate device, initiate data transfers and perform other auxiliary tasks. The host code has been omitted for clarity as with around 50 lines of code it is considerably longer than the actual kernel code. 
