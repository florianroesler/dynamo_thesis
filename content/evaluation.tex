...
\begin{figure}[H]
	...
	\centering
	\caption{Local Job Suite Benchmark Results}
...
In this scenario all utilized resources are located at a cloud service like Amazon's EC2. One central management node is run that does not execute any computations but distributes the tasks instead. The instance type for the management node is chosen based on the availability of a 10 Gbit/s interconnect. For the actual computations two different cluster sets are used. First all benchmarks are run on a CPU cluster that is comprised of of a varying number of c4.8xlarge instances. Then all tests are run again on g2.2xlarge instances, which provide a NVIDIA GRID K520 GPU. This instance type is marked by Amazon as having 'high' network capabilities. In tests conducted with iperf3 the bandwidth was determined to vary within an interval between 986 Mbit/s and 998 Mbit/s. Although the g2.2xlarge instances include an Intel Xeon CPU, which is also available through OpenCL, they are deactivated to form a pure GPU cluster.
