...
For both instance types the network speeds are measured by launching the iperf3 and ping from within the EC2 on a c4.8xlarge instance.
...
\begin{table}[!htb]
	...
	\begin{adjustbox}{width=0.5\textwidth}
		...
		\begin{tabular}{l | l | l}
			~                     & c4.8xlarge               			& g2.2xlarge                \\
			\hline
			iperf3                & 9.25 Gbit/s ($\sigma = 0.026$) 	& 992.6 MBit/s ($\sigma = 4.8$) \\
			ping                  & 0.129 ms ($\sigma = 0.023$)  		& 1.29 ms ($\sigma = 0.047$)  \\
		\end{tabular}
	\end{adjustbox}
