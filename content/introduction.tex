...
The following chapters are organized as follows. First in chapter \textit{\nameref{background}} relevant parallelization and distribution technologies that are discussed throughout this research are introduced. Chapter \textit{\nameref{related}} showcases various existing research efforts for parallel computations on single machines as well as clusters. In \textit{\nameref{benchmarking_methodology}} the general measurement process for this research is explained, including the hardware setup and chosen workloads. Chapter \textit{\nameref{main}} covers the introduction of a new framework that is contributed through this thesis, called Dynamic OpenCL. Following explanations for the implemented solutions, chapter \textit{\nameref{evaluation}} includes various performance tests for the framework in different cluster environments. In chapter \textit{\nameref{demonstration}} a reference implementation based on Dynamic OpenCL is showcased that allows users to start cluster computations through a webserver. Ultimately this thesis finishes with \textit{\nameref{conclusion}}, which summarizes the achieved research goals and illustrates future work that aims to overcome current limitations of Dynamic OpenCL.