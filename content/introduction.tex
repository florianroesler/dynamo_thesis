...
\section{Goals}
\label{goals}
While there exists a multitude of solutions to solve the problems mentioned in \ref{motivation}, many of them only cover partial problems. This research aims at providing a framework to programmers that solves all the given problems by reusing existing solutions. Therefore the goal is to meaningfully combine present partial software solutions and add individual functionality where necessary. Thus, the following requirements are defined in order to streamline the development:
